...
def createLeague(username, leagueName, leaguePassword, maxTeams, sportID, teamName, privateKey, maxC, maxLW, maxRW, maxD, maxG, statsList):
    ...
    cursor.execute("INSERT INTO rules(max_c, max_lw, max_rw, max_d, max_g, stats_list) VALUES(%s, %s, %s, %s, %s, %s)", (maxC, maxLW, maxRW, maxD, maxG, statsList))
    db.commit()
    ...
    query = "INSERT INTO league(league_name, league_pass, commissioner_id, rules, max_teams, sport_id, private, salt) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)"
    cursor.execute(query, (leagueName, hex_dig, participantID[0], rulesID[0], maxTeams, sportID, privateKey, salt))
    db.commit()
...
createLeague(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10], sys.argv[11], sys.argv[12], sys.argv[13])
